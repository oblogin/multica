...
export const INSTALL_RUNTIME_ISSUE_TITLE = {
  ...
  ru: "Подключите среду выполнения, чтобы начать работу с Mika",
} as const;
...
const ru = `Добро пожаловать в Multica.

Чтобы агенты могли выполнять задачи, подключите среду выполнения. Пока вы её устанавливаете, Multica можно использовать для управления проектами.
...
4. Проверьте, что терминал находит программу:
   codex --version
5. Подождите, пока Multica обнаружит её. Работающий демон проверяет новые CLI каждые несколько минут, поэтому обычно перезапуск не нужен. Чтобы обновить список сразу, выполните:
   multica daemon restart
   В настольном приложении откройте любую локальную среду выполнения и нажмите «Перезапустить». Простое закрытие и повторное открытие приложения не перезапускает демон: он продолжает работать в фоне.
6. Откройте страницу сред выполнения и обновите её. Codex должен появиться в сети.
7. На этой странице выберите «Начать с Mika», чтобы создать Mika и открыть первый чат.

Справка по Codex: https://developers.openai.com/codex/cli

Mika поможет превратить вашу цель в задачу, запустит подходящего агента и предложит повторно используемые решения для вашего процесса.`;
